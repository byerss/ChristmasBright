...
int eveningOffset = 20;   //20 MINTUES AFTER SUNSET
int morningOffset = -20;  //20 MINUTES BEFORE SUNRISE

//MORNING ON TIME
int amONtime = 630;

//EVENING OFF TIME
int pmOFFtime = 2300; //11PM
...
//RTC MUST BE SET TO PST FOR SUN DATA TABLES TO WORK PROPERLY
 ...
 void loop() {
  ...
  //TURN ON BETWEEN (amONtime UNTIL (morningOffset) MINUTES FROM SUNRISE
   ...
   Serial.println(nowAbs);
   Serial.println(riseAbs);
   Serial.println(setAbs);
   Serial.println(amONtimeAbs);
   Serial.println(pmOFFtimeAbs);
   delay(20000);
